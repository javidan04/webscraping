...
def newsLists():
    ...
    news = []
    try:
        results = soup.find("div", class_="ajax-data-block")
        job_elements = results.find_all("li")

        for job_element in job_elements:
            item = dict()
            try:
                titleTag = job_element.find("strong", class_="title")
                hrefTag = job_element.find("a")
                dateTag = job_element.find("span", class_="time")
                categoryTag = False
                ...
                item["category"] = categoryTag.text if categoryTag else ""
...
def read_news_items(news):
    ...
    soup = BeautifulSoup(page.content, "html.parser")
    try:
        results = soup.find("div", class_="quiz-holder")
        titleTag = results.find("h1")
        category =  results.find("span", class_="category")
        contet = results.find("div", class_="article_text")
        dateTag = results.find("div", class_="date-info")
        imgTag = results.find("img", class_="content-img")
...
def read():
    ...
    not_read = session.query(Tables.News) \
        .filter(
        and_(
            Tables.News.link.like('%news.milli.az%'),
