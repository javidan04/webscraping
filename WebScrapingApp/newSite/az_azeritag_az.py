...
def newsLists():
    URL = "https://azertag.az/son-xeberler"
    page = requests.get(URL)
    soup = BeautifulSoup(page.content, "html.parser")
    news = []
    try:
        results = soup.find("div", class_="content-inner clear")
        job_elements = results.find_all("div", class_="news-item")

        for job_element in job_elements:
            item = dict()
            try:
                titleTag = job_element.find("h1", class_="news-title")
                hrefTag = job_element.find("a")
                dateTag = job_element.find("div", class_="news-date")
                item["title"] = titleTag.text if titleTag else None
                item["href"] = "https://azertag.az"+hrefTag["href"] if hrefTag else None
...
def read_news_items(news):
    ...
    try:
        results = soup.find("div", id="content-article")
        ...
        item["title"] = titleTag.text if titleTag else None
        item["img"] = "https://azertag.az/"+imgTag["src"] if imgTag else None
        item["date"] = dateTag.text if dateTag else None
...
def read():
    ...
    not_read = session.query(Tables.News) \
        .filter(
        and_(
            Tables.News.link.like('%azertag.az%'),
